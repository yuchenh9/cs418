...
def draw(tri):
    ...
    basecolor=(r[low_index],g[low_index]  ,b[low_index] ,alpha[low_index])
   #print(f'{r[up_index]-r[low_index]},{g[up_index]-g[low_index]},{b[up_index]-b[low_index]}')
    W1=(r[up_index]-r[low_index],g[up_index]-g[low_index],b[up_index]-b[low_index] ,alpha[up_index]-alpha[low_index])
    W2=(r[middle_index] -r[low_index] ,g[middle_index] -g[low_index] ,b[middle_index] -b[low_index] ,alpha[middle_index] -alpha[low_index])
    #print(f'W1={W1}')
    ...
    for i in range(len(line_a)):
        ...
        while condition:#
            ...
            #print(f'x,y:({x_value},{round(line_a[i][1])})')
           ...
           #print(f'{(x_viewport[low_index],y_viewport[low_index])},{(x_viewport[middle_index],y_viewport[middle_index])},{(x_viewport[up_index],y_viewport[up_index])},{p}')
            ...
            if depth_flag==True:
                ...
                #print(f'append{_r},{_g},{_b},{_depth},to ({_x},{_y})')
                pixels[_y][_x].append([_r,_g,_b,_a,_depth])
                (testx,testy)=(2,3)
           #     if _x==testx and _y==testy:
                    #print(f'test{(testx,testy)}{pixels[_y][_x]}')
            else:
                #print("yes")
                #print(f'append{_r},{_g},{_b},to {_x},{_y}')
                pixels[_y][_x][-1]=[(_r),(_g),(_b),1,_depth]
...
def drawt(tri):
    ...
    W1_w=w[up_index]-base_w
    W2_w=w[middle_index]-base_w
    basecolor=[t_x[low_index],t_y[low_index]]
   #print(f'{r[up_index]-r[low_index]},{g[up_index]-g[low_index]},{b[up_index]-b[low_index]}')
    W1=[t_x[up_index]-t_x[low_index],t_y[up_index]-t_y[low_index]]
    W2=[t_x[middle_index] -t_x[low_index] ,t_y[middle_index] -t_y[low_index]]
    #print(f'W1={W1}')
    #(unit_x,unit_y)=getLerpLinearUnit((x_viewport[low_index],y_viewport[low_index]),(x_viewport[middle_index],y_viewport[middle_index]),(x_viewport[up_index],y_viewport[up_index]))
    #print(f'{unit_x},{unit_y}')
    for i in range(len(line_a)):
        ...
        while condition:#
           ...
           #print(f'{(x_viewport[low_index],y_viewport[low_index])},{(x_viewport[middle_index],y_viewport[middle_index])},{(x_viewport[up_index],y_viewport[up_index])},{p}')
            ...
            #print(f'base{basecolor}+{W1}*{w1}+{W2}*{w2}')
            _t_x=basecolor[0]+W1[0]*w1+W2[0]*w2
            _t_y=basecolor[1]+W1[1]*w1+W2[1]*w2
            #print(f'(_t_x,_t_y)=({_t_x},{_t_y})')
            #print(f'{basecolor[0]}+{W1[0]}*{w1}+{W2[0]}*{w2}')
            if hyp:
                _t_x=_t_x/_w
                _t_y=_t_y/_w
                _depth=_depth/_w
            #print(f'hyp(_t_x,_t_y)=({_t_x},{_t_y})')
            _r,_g,_b,_a=texture.getpixel((_t_x%1*texture_width, _t_y%1*texture_height))
...
try:
    with open(input_file, 'r') as file:
        ...
        for line in lines:
            ...
            if newwords[0]=='texture':
                ...
                texture = Image.open(newwords[1])
                
                texture_width, texture_height = texture.size
            if newwords[0]=='decals':
                decals=True
            if newwords[0]=='texcoord':
                texcoord=[float(newwords[1]),float(newwords[2])]
                #print(newwords[1]+newwords[2])
                #print(f'append{texcoord}')
            if newwords[0]=='cull':
                cull=True
            if newwords[0]=='xyzw' :
                _x=float(newwords[1])
                ...
                t_x.append(texcoord[0])
                t_y.append(texcoord[1])
                alpha.append(current_color[3])
            ...
            if newwords[0]=='trit' :
                ...
                for element in newwords[1:4]:
                    element=int(element)
                    if element>0:
                        trit.append(element-1)
                    else :
                        if element<0:
                            trit.append(len(x)+element)
                ...
                #draw(tri)
            if newwords[0]=='rgba':
                current_color=(int(newwords[1]),int(newwords[2]),int(newwords[3]),float(newwords[4]))
            if newwords[0]=='rgb' :
                current_color=(int(newwords[1]),int(newwords[2]),int(newwords[3]),1)
    ...
    #pixels=[[[[0,0,0,0,10]]] * width for _ in range(height)]#rgba
    pixels=[[[[1,1,1,1,10]] for _ in range(width)] for _ in range(height)]
    if cull:
        index=0
        for tri in tris:
    ...
    for tri in trits:
        drawt(tri)
    #print(f'after draw(){pixels[4]}')
    #print(t_x)
    def blendPixel(array_of_pixels):
        target=array_of_pixels[0]
        for pixel_index in range(len(array_of_pixels)):
            if pixel_index==0:
                continue
            a1=array_of_pixels[pixel_index][3]+(1-array_of_pixels[pixel_index][3])*target[3]
            for i in range(3):
                target[i]=(((1-array_of_pixels[pixel_index][3])*target[3])/a1)*target[i]+(array_of_pixels[pixel_index][3]/a1)*array_of_pixels[pixel_index][i]
            target[3]=a1
            target[4]=array_of_pixels[-1][4]
        return target
    
    for x in range(len(pixels[0])):
        for y in range(len(pixels)):
            pixels[y][x].sort(key=lambda x: x[4], reverse=True)
            pixels[y][x]=blendPixel(pixels[y][x])
    with open('bugbug.txt', 'w') as f:
    ...
    if fsaa >0:
        pixels_after_fsaa=[[[1,1,1,1]] * original_width for _ in range(original_height)]
        #txt=[[[1,1,1,1]] * original_width for _ in range(original_height)]
        for x1 in range(original_width):
            for y1 in range(original_height):
                increment_count=0
                base=[0,0,0,0]
                for x2 in range(fsaa):
                    for y2 in range(fsaa):
                        #print((start_x+_x,start_y+_y))
                        
                        for i in range(3):
                            base[i]+=pixels[y1*fsaa+y2][x1*fsaa+x2][i]*pixels[y1*fsaa+y2][x1*fsaa+x2][3]

                        increment_count+=pixels[y1*fsaa+y2][x1*fsaa+x2][3]

                        base[3]+=pixels[y1*fsaa+y2][x1*fsaa+x2][3]*1
                ...
                base[3]=base[3]/(fsaa*fsaa)
                if increment_count>0:
                    for i in range(3):
                        base[i]=base[i]/increment_count
                pixels_after_fsaa[y1][x1]=base
                        

    else:   
        pixels_after_fsaa=copy.deepcopy(pixels)
    for y in range((len(pixels_after_fsaa))):
        for x in range(len(pixels_after_fsaa[0])):
            if sRGB or fsaa>0:
